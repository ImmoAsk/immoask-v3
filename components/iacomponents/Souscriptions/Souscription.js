...
const updateNegotiation = async ({ negociationOffer, statut }) => {
    ...
    try {
        const response = await fetch(
            `${API_URL}?query=mutation{updateVisitationRequest(input:{id:${Number(negociationOffer.id)},statut:${statut}}){statut}}`
        );
        if (!response.ok) {
                ...
                <Card.Body>
                    ...
                    <h3 className="h6 card-title pt-1 mb-3">
                        <p className="text-nav ext-decoration-none">
                            <strong>
                                {project.fullname_visitor ? project.fullname_visitor : project.client?.name}
                            </strong> {role === '1200' && (project.telephone_visitor ? "+" + project.telephone_visitor : "")} souhaite payer la souscription de la propriété No. {project.propriete.nuo}
                        </p>
                    </h3>
                    <div className="fs-sm">
                        <span className="text-nowrap me-3">
                            <i className="fi-calendar text-muted me-1"></i>
                            {formatDate(project.date_transaction)}
                        </span>
                    </div>

                    {/* Show Accept and Decline buttons when project.statut === 0 */}
                    { project.client.id === userId && project.statut === 0 && (
                        <>
                            <div className="d-flex justify-content-center mt-3">
                                <button
                                    className="btn btn-primary flex-grow-1"
                                    onClick={console.log("Accepting project", project)}
                                >
                                    Payer maintenant
                                </button>
