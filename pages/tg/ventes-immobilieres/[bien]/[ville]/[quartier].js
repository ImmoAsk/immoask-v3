...
import OffCanvasFilter from '../../../../../components/iacomponents/FilterSearch/OffCanvasFilter'
  ...
  const CatalogPage = ({ _rentingProperties, bienId, villeId, quartierId, soffreId }) => {
    ...
    const { bien,ville,quartier } = router.query;

    const ocategory = bienId;
    const ooffer = soffreId;
    const oville = villeId;
    const oquartier = quartierId;

    console.log(oville);
    console.log(oquartier);
    console.log(ooffer);
  ...
  const handleFilterSubmit = (data) => {
    ...
    export async function getServerSideProps(context) {
      ...
      try {
        ...
        const [villeResponse, quartierResponse, bienResponse] = await Promise.all([
          fetch(
            `${baseUrl}?query={getTownIdByTownName(minus_denomination:"${ville?.toLowerCase()}"){denomination,id,code}}`
          ),
          fetch(
            `${baseUrl}?query={getDistrictIdByDistrictName(minus_denomination:"${quartier?.toLowerCase()}"){denomination,id,code,minus_denomination}}`
          ),
          fetch(
        ...
        // Parse JSON
        const [villeData, quartierData, bienData] = await Promise.all([
          villeResponse.json(),
          quartierResponse.json(),
          bienResponse.json(),
        ]);
    
        const villeId = villeData?.data?.getTownIdByTownName?.id || null;
        const quartierId = quartierData?.data?.getDistrictIdByDistrictName?.id || null;
        const bienId = bienData?.data?.getCategoryIdByCategorieName?.id || null;
