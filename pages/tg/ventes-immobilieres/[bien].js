    ...
    >
      ...
      <Container fluid className='mt-5 pt-5 p-0'>
        <Row className='g-0 mt-n3'>
          ...
          <OffCanvasFilter show={show} handleClose={handleClose} isDesktop={isDesktop} onFilterSubmit={handleFilterSubmit}/>
...
/* export async function getServerSideProps(context) {
  ...
  let dataAPIresponse = await fetch(`https://immoaskbetaapi.omnisoft.africa/public/api/v2?query={getPropertiesByKeyWords(limit:10,orderBy:{column:NUO,order:DESC},offre_id:"2",categorie_id:"${bienId}")
...
export async function getServerSideProps(context) {
  const { bien } = context.query;
  const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || API_URL;

  if (!baseUrl) {
    console.error('API URL not configured. Check NEXT_PUBLIC_API_BASE_URL in .env');
    return { props: { _rentingProperties: [], soffreId: null, bienObject: null } };
  }
  if (!bien) {
    return { props: { _rentingProperties: [], soffreId: null, bienObject: null } };
  }
